const CookieUtil = {
   ...
   */
  getCookies: function () {
    return document.cookie.split("; ").reduce((prev:any, cookieStr) => {
   ...
   * @param {number} [days] - 过期天数,设置cookie15天过期
   * @param {string} [path] - Cookie 路径
   */
  setCookie: function (name:string, value:string, days=15, path = "/") {
   ...
   * @param {string} [path] - Cookie 路径
   */
  deleteCookie: function (name:string, path = "/") {
